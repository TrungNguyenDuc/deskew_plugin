...
class OpmFrame(ttk.Frame):
    ...
    def _setup_initial_image_for_display(self, image_path, display_type="mip", autofit=False):
        ...
        original_pil_attr = f"{display_type}_original_pil_image"; original_np_attr = f"{display_type}_original_image_data_np"
        min_val_attr = f"{display_type}_image_data_min_val"; max_val_attr = f"{display_type}_image_data_max_val"
        zoom_attr = f"{display_type}_current_zoom_level"; view_rect_attr = f"{display_type}_view_rect_on_original"
        tk_image_attr = f"{display_type}_tk_image"
        try:
            self._log_to_gui_if_possible(f"GUI: Loading {display_type.upper()} image: {image_path}", "DEBUG")
            if not str(image_path).lower().endswith((".tif", ".tiff", ".png")): self._handle_display_error(f"Expected TIFF/PNG, got: {os.path.basename(str(image_path))}", display_type); return
            pil_image = Image.open(image_path); pil_image.load(); setattr(self, original_pil_attr, pil_image)
            if display_type == "mip":
                np_data = np.array(pil_image); setattr(self, original_np_attr, np_data)
                if np_data.size > 0:
                    ...
                    self.min_display_slider.config(from_=min_val, to=max_val); self.max_display_slider.config(from_=min_val, to=max_val)
                    self.vars["display_min_slider"].set(min_val); self.vars["display_max_slider"].set(max_val)
                else: raise ValueError("Loaded MIP image array empty.")
            if autofit: self._autofit_image_to_label(getattr(self, original_pil_attr), target_label, zoom_attr, view_rect_attr)
            else: setattr(self, zoom_attr, 1.0); img_w, img_h = getattr(self, original_pil_attr).size; setattr(self, view_rect_attr, (0, 0, img_w, img_h))
            self._update_displayed_image_portion(display_type)
        except Exception as e: self._handle_display_error(f"Error loading initial {display_type} image: {e}\n{traceback.format_exc()}", display_type)
    ...
    def _clear_displayed_image(self, display_type="mip"):
        ...
        target_label.configure(image='',text=default_text); setattr(self,f"{display_type}_tk_image",None); setattr(self,f"{display_type}_original_pil_image",None)
        setattr(self,f"{display_type}_current_zoom_level",1.0); setattr(self,f"{display_type}_view_rect_on_original",None)
        if display_type=="mip":
            setattr(self,f"{display_type}_original_image_data_np",None); setattr(self,f"{display_type}_image_data_min_val",0); setattr(self,f"{display_type}_image_data_max_val",255)
            self.min_display_slider.config(from_=0,to=255);self.max_display_slider.config(from_=0,to=255)
            self.vars["display_min_slider"].set(0);self.vars["display_max_slider"].set(255);self.vars["display_actual_min_max_label"].set("Img Min/Max: N/A")
